...
function parseDuration(t: string): number {
  ...
  if (dMatch) mins += parseInt(dMatch[1]) * 1440;
...
function TrendIndicator({ trend }: { trend: "widening" | "narrowing" | "stable" }) {
  ...
  return <span className="text-xs text-white/30">→ stable</span>;
}
...
export default function ArbsPage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        if (!cancelled) setError(err.message ?? "Failed to load arbs");
      } finally {
  ...
  const maxVisible = limits.arbsVisible;
  const visibleArbs = filteredArbs.slice(0, maxVisible === Infinity ? filteredArbs.length : maxVisible);
  const gatedArbs = maxVisible === Infinity ? [] : filteredArbs.slice(maxVisible);
  const blurPreviewArbs = gatedArbs.slice(0, 3); // Show 3 blurred rows as teaser
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      {/* Top Arb Cards - always show top 3 */}
      {allArbs.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      ...
      {isFree && gatedArbs.length > 0 && (
        <InlineUpgradeBanner
          requiredTier="pro"
          message={`You're seeing ${visibleArbs.length} of ${filteredArbs.length} arbitrage opportunities. Upgrade to Pro for full access.`}
          count={gatedArbs.length}
        />
      )}
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {(["all", "1", "2", "3", "5"] as SpreadFilter[]).map((s) => (
            <button key={s} onClick={() => setSpreadFilter(s)} className={pillClass(spreadFilter === s)}>
              {s === "all" ? "Any" : `>${s}%`}
            </button>
          ))}
          ...
          {([
            ...
            <button key={val} onClick={() => setDirectionFilter(val)} className={pillClass(directionFilter === val)}>
              {label}
            </button>
          ))}
        </div>
        <div className="text-xs text-white/30">
          Showing {visibleArbs.length} of {totalCount} opportunities
          {isFree && totalCount > (limits.arbsVisible as number) && (
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-white/10 hover:bg-transparent">
                  ...
                  <TableHead className="text-white/40 text-xs text-center cursor-pointer hover:text-white/70 transition-colors select-none" onClick={() => handleSort("platforms")}>
                    Platforms <SortArrow column="platforms" />
                  </TableHead>
                  <TableHead className="text-white/40 text-xs text-center cursor-pointer hover:text-white/70 transition-colors select-none" onClick={() => handleSort("rawSpread")}>
                    Raw Spread <SortArrow column="rawSpread" />
                  </TableHead>
                  <TableHead className="text-white/40 text-xs text-center cursor-pointer hover:text-white/70 transition-colors select-none" onClick={() => handleSort("adjustedSpread")}>
                  ...
                  <TableHead className="text-white/40 text-xs text-center cursor-pointer hover:text-white/70 transition-colors select-none" onClick={() => handleSort("trend")}>
                    Trend <SortArrow column="trend" />
                  </TableHead>
                  <TableHead className="text-white/40 text-xs text-center cursor-pointer hover:text-white/70 transition-colors select-none" onClick={() => handleSort("timeOpen")}>
              ...
              <TableBody>
                ...
                {/* Blurred preview rows for gated arbs */}
                {blurPreviewArbs.map((arb) => (
                  <TableRow
                    key={arb.id}
              ...
              <div className="absolute bottom-0 left-0 right-0 h-32 flex items-center justify-center bg-gradient-to-t from-[#0a0a0a] via-[#0a0a0a]/80 to-transparent">
                <div className="text-center">
                  ...
                  <p className="text-sm text-white/60 mb-2">
                    Upgrade to see all {filteredArbs.length} arbitrage opportunities
                  </p>
                  <Link
                    href="/pricing"
          ...
          <div className="p-8 text-center space-y-6">
            <div>
              ...
              <p className="text-sm text-white/40 mt-2 max-w-lg mx-auto">
                Free users see the top {limits.arbsVisible} arbs. Pro unlocks everything in real-time.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-2xl mx-auto">
              <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-4">
                ...
                <div className="text-xs text-white/30 mt-1">See arbs the moment they appear.</div>
              </div>
              <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-4">
                <div className="text-2xl mb-2">📊</div>
                <div className="text-sm font-medium text-white/80">Unlimited arbs</div>
                <div className="text-xs text-white/30 mt-1">Access every opportunity, not just {limits.arbsVisible}.</div>
              </div>
              <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-4">
                <div className="text-2xl mb-2">📈</div>
                <div className="text-sm font-medium text-white/80">Full history</div>
                <div className="text-xs text-white/30 mt-1">90-day spread history and P&L tracking.</div>
              </div>
            </div>
