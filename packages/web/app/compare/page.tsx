...
export default function ComparePage() {
  const [selected, setSelected] = useState<MatchedMarket | null>(null);
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-white/10 hover:bg-transparent">
                ...
                <TableHead className="text-white/40 text-xs text-center">Spread</TableHead>
                <TableHead className="text-white/40 text-xs text-center">Volume</TableHead>
                <TableHead className="text-white/40 text-xs text-center">Confidence</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {matchedMarkets.map((market) => (
                <TableRow
                  ...
                  onClick={() => setSelected(selected?.id === market.id ? null : market)}
                >
                ...
                <div className="mt-6 pt-4 border-t border-white/5">
                  ...
                  <div className="flex items-end gap-px h-16">
                    {selected.spreadHistory.map((s, i) => (
                      <div
                        key={i}
                        className="flex-1 bg-emerald-500/30 rounded-t-sm hover:bg-emerald-500/50 transition"
                        style={{ height: `${Math.min(100, s.spread * 10)}%` }}
                        title={`${s.date}: ${s.spread.toFixed(1)}%`}
                      />
                    ))}
                  </div>
                </div>
